...
contract WithdrawalFacet is Modifiers, IWithdrawable {
    ...
    function withdraw(address recipient, address token, uint256 id, uint256 amount) external override payable {
        ...
        if(token == address(0)) {
            ...
            if(isErc20) {
                ...
                require(ERC20(token).balanceOf(this) >= amount, "insufficient funds");
                ERC20(token).transfer(recipient, amount); // transfer the token
            ...
            if(isErc721) {
                ...
                require(ERC721(token).balanceOf(this) >= amount, "insufficient funds");
                ERC721(token).transferFrom(this, recipient, amount); // transfer the token
            ...
            if(isErc1155) {
                ...
                require(ERC1155(token).balanceOf(this) >= amount, "insufficient funds");
                ERC1155(token).safeTransferFrom(this, recipient, id, amount, ""); // transfer the token
                emit TokenWithdrawn(recipient, token, id, amount);
                return;
            }
