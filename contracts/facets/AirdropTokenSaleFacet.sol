...
contract AirdropTokenSaleFacet is ITokenSale, Modifiers {
    ...
    function airdropRedeemed(
        ...
        // mint the token
        address targetToken = s
            .airdropTokenSaleStorage
            ._tokenSales[tokenSaleId]
            .token;
        if (InterfaceChecker.isERC20(targetToken)) {
            IERC20Mint(targetToken).mintTo(recipient, amount);
        } else if (InterfaceChecker.isERC721(targetToken)) {
            tokenHash_ = IERC721Mint(address(this)).mintTo(
                recipient,
                tHash
            );
        } else if (InterfaceChecker.isERC1155(targetToken)) {
