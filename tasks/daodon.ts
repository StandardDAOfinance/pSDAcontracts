...
task("get-token-power", "get token power")
  ...
  .setAction(async ({ token }, hre: HardhatRuntimeEnvironment) => {
    ...
    const tx = await tokenMinterContract.getAttribute(token, "Power");
...
task("staked-token-power", "stake a token")
  ...
  .setAction(async ({ token }, hre: HardhatRuntimeEnvironment) => {
    // the generative token factory
    const attributeMutationPoolFacet = await getDiamondFacet(
      hre,
      "AttributeMutationPoolFacet"
    );
    const tx = await attributeMutationPoolFacet.getAccruedValue(token);
